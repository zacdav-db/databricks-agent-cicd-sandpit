...
if jq -e '.apps | index("mcp") != null' <<<"${selection}" >/dev/null; then
  scripts/retire_replaced_app.sh \
    "$("${python_bin}" scripts/app_names.py mcp "${target}")" \
    "$("${python_bin}" scripts/app_names.py legacy-mcp "${target}")"
fi

if jq -e '.apps | index("langchain") != null' <<<"${selection}" >/dev/null; then
  scripts/retire_replaced_app.sh \
    "$("${python_bin}" scripts/app_names.py langchain "${target}")" \
    "$("${python_bin}" scripts/app_names.py legacy-langchain "${target}")"
fi

while IFS= read -r agent_name; do
  scripts/retire_replaced_app.sh \
    "$(
      "${python_bin}" scripts/app_names.py \
        generated "${target}" "${agent_name}"
    )" \
    "$(
      "${python_bin}" scripts/app_names.py \
        legacy-generated "${target}" "${agent_name}"
    )"
done < <(jq -r '.agents[]' <<<"${selection}")
