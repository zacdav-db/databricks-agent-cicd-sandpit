...
# agent-prod- is 11 characters and Databricks App names are limited to 30.
...
def _app_resource(
    ...
    *,
    ...
    return {
        "name": f"agent-${{var.resource_prefix}}-{agent.name}",
        "description": (
...
def _bundle_config(agent: AgentDefinition) -> dict[str, Any]:
    """Return a complete, independently stateful DAB for one agent App."""
    variables = {
        "catalog": {"default": "zacdav_sandpit_catalog"},
        "schema": {},
        "resource_prefix": {},
    ...
    targets = {
        target: {
            ...
            "variables": {
                ...
                "trace_table_prefix": f"{target}_sandpit_agent_cicd",
            },
            "workspace": {
...
def _legacy_bundle_config(agents: list[AgentDefinition]) -> dict[str, Any]:
    """Describe the former shared state so CI can transfer existing Apps safely."""
    fixed_apps = {
        "langchain_agent": "${var.resource_prefix}-sandpit-langchain-agent",
        "mcp_server": "${var.resource_prefix}-sandpit-mcp-tools",
        "omnigent": "${var.resource_prefix}-sandpit-omnigent",
    }
    targets = {
        target: {
            "mode": "development" if target == "dev" else "production",
            "variables": {"resource_prefix": target},
            "workspace": {
                "host": "https://fevm-zacdav-sandpit.cloud.databricks.com",
                "root_path": (
                    "/Workspace/Users/${workspace.current_user.userName}"
        ...
        "resources": {
            "apps": {
                **{
                    key: {
                        "name": name,
                        "source_code_path": f"./placeholder/{key}",
                    }
                    for key, name in fixed_apps.items()
                },
                **{
                    agent.resource_key: {
                        "name": f"${{var.resource_prefix}}-agent-{agent.name}",
                        "source_code_path": f"./placeholder/{agent.name}",
                    }
                    for agent in agents
                },
            },
...
def compose(root: Path) -> dict[str, Any]:
    """Generate one independently stateful DAB per agent folder atomically."""
    root = root.resolve()
    ...
    try:
        bundles_dir = temporary / "bundles"
        bundles_dir.mkdir()
        legacy_bundle_dir = temporary / "legacy-shared-bundle"
        legacy_bundle_dir.mkdir()
        legacy_placeholder_dir = legacy_bundle_dir / "placeholder"
        legacy_placeholder_dir.mkdir()
        for name in ("langchain_agent", "mcp_server", "omnigent"):
            (legacy_placeholder_dir / name).mkdir()
        for agent in agents:
            (legacy_placeholder_dir / agent.name).mkdir()
        ...
        for agent in agents:
            bundle_dir = bundles_dir / agent.name
            destination = bundle_dir / "app"
            bundle_dir.mkdir()
            shutil.copytree(
                agent.source,
                destination,
            ...
            (bundle_dir / "databricks.yml").write_text(
                yaml.safe_dump(_bundle_config(agent), sort_keys=False),
                encoding="utf-8",
            )
            index_agents.append(
                {
                    ...
                    "app_name": f"agent-${{var.resource_prefix}}-{agent.name}",
